...
ISO_NAME="ps-xubuntu-14.04.2-server-amd64-${BUILD}.iso"
PS_LABEL="PS Xubuntu Server"
...
GRUB_CFG_X="${NEW_DIR}/boot/grub/grub.cfg.x"

TXT_CFG="${NEW_DIR}/isolinux/txt.cfg"
TXT_CFG_X="${NEW_DIR}/isolinux/txt.cfg.x"

echo "PS - Remove old \"${GRUB_CFG}\""
rm -f ${GRUB_CFG}
echo "PS - Make sure \"${GRUB_CFG_X}\" is active"
cp -f ${GRUB_CFG_X}  ${GRUB_CFG}


echo "PS - Remove old \"${TXT_CFG}\""
rm -f ${TXT_CFG}
echo "PS - Make sure \"${TXT_CFG_X}\" is active"
cp -f ${TXT_CFG_X} ${TXT_CFG}
...
cp ${ISO_NAME} /media/sf_D_DRIVE/My_ISOs/3_Ubuntu/ps-custom/
echo "PS - Finished"
